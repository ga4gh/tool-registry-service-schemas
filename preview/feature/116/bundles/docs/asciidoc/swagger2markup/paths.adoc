...
==== Description
Get a list of objects that contain the relative path and file type. The descriptors are intended for use with the /tools/{id}/versions/{version_id}/{type}/descriptor/{relative_path} endpoint. If format=zip is specified, a zip file of all files available will be gotten instead.
...
__required__|An identifier of the tool version for this particular tool registry, for example `v1`.|string
|**Query**|**format** +
__optional__|specify format=zip to download a zip file of all files available|enum (zip)
|===
