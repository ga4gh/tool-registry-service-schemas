...
|Name|Description|Schema
|**hashcode** +
__optional__|Helpful for immutable tool versions, but can be independent. This exposes the hashcode for specific image versions to verify that the container version pulled is actually the version that was indexed by the registry. +
**Example** : `""`|string
|**image_name** +
...
|**immutable** +
__optional__|This version of a tool is guaranteed to not change over time (as opposed to a tool build from a develop or master branch in git).|boolean
|**meta_version** +
