...
**Example** : `"v1"`|string
|**image** +
__optional__|The docker path to the image (and version) for this tool +
**Example** : `"quay.io/seqware/seqware_full/1.1"`|string
|**image_name** +
__optional__|Used in conjunction with a registry_url if provided to locate images|string
|**meta_version** +
...
|**registry_url** +
__optional__|A URL to a Singularity registry is provided when a specific type of image does not use ids in the Docker format. Used along with image_name to locate a specific image.|string
|**url** +
__required__|The URL for this tool in this registry +
